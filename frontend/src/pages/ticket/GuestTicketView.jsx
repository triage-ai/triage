import { Alert, AppBar, Avatar, Box, Button, Chip, createTheme, Divider, IconButton, List, ListItem, ListItemAvatar, ListItemText, Skeleton, Stack, styled, Toolbar, Typography } from "@mui/material"
...
import logo from '../../assets/logo-horizontal-primary.svg';
...
const theme = createTheme({
  ...
  const TicketHeader = ({
	...
	createdDate = "2023-04-20T10:30:00Z",
	updatedDate = "2023-04-21T14:45:00Z",
	...
	return (
	  <StyledAppBar elevation={0} minWidth={'100vh'}>
		<Toolbar>
		  {/* Logo area */}
		  ...
		  <Box sx={{ flexGrow: 1, textAlign: "center" }}>
			...
			<Typography variant="caption" display="block" color="text.secondary">
			  Created: {relativeTime(dayjs.utc(createdDate).local())} • Updated: {relativeTime(dayjs.utc(updatedDate).local())}
			</Typography>
		  </Box>
...
export const GuestTicketView = () => {
	...
	const { getTicketByNumberForUser, getTicketByNumberForGuest } = useTicketBackend()
    ...
    const { createThreadEntryForGuest, createThreadEntryForUser } = useThreadsBackend();
    const { getPresignedURL } = useAttachmentBackend();
    const { userAuthState, guestAuthState, guestLogout } = useContext(AuthContext);
    const [confirmation, setConfirmation] = useState('')
    const type = useMemo(() => (userAuthState.isAuth === true ? 'user' : 'guest'), [guestAuthState, userAuthState])
	...
	useEffect(() => {
		const refreshTicket = type === 'user' ? refreshTicketForUser : refreshTicketForGuest;
		refreshTicket()
        setLoading(false)
    }, [])

	const refreshTicketForUser = () => {
        getTicketByNumberForUser(number)
            .then(res => {
    ...
    const handleSubmit = async () => {
        ...
        const threadEntryCreate = type === 'user' ? createThreadEntryForUser : createThreadEntryForGuest;
